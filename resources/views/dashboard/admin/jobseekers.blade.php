...
@section('content')
    <h1 class="text-3xl font-bold mb-6">Manage Jobseekers</h1>
    ...
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
        <table class="min-w-full leading-normal">
            ...
            <tbody id="jobseekersTableBody"></tbody>
        </table>
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    const tableBody = document.getElementById('jobseekersTableBody');
    ...
    async function fetchJobseekers(page = 1, searchTerm = '') {
        try {
            let endpoint = `/jobseekers?page=${page}`;
            if (searchTerm) endpoint += `&search=${encodeURIComponent(searchTerm)}`;
            const response = await apiRequest(endpoint);
            tableBody.innerHTML = '';
            if (!response.data || response.data.length === 0) {
                tableBody.innerHTML = `<tr><td colspan="3" class="text-center py-4">No jobseekers found.</td></tr>`;
                paginationControls.innerHTML = '';
                return;
            }
            response.data.forEach(user => {
                const profileUrl = `{{ route('admin.jobseeker.profile', ['id' => ':id']) }}`.replace(':id', user.id);
    ...
    searchInput.addEventListener('input', () => {
        ...
        debounceTimer = setTimeout(() => {
            fetchJobseekers(1, searchInput.value);
        }, 500);
    });

    paginationControls.addEventListener('click', function(e) {
        if (e.target.classList.contains('pagination-btn') && e.target.dataset.page) {
            fetchJobseekers(e.target.dataset.page, searchInput.value);
        }
    });
